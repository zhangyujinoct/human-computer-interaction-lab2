 ...
 **/
public class RowLayout extends BaseLayoutVisualElement {
    ...
    public void doLayout() {
        ...
        if (!this.visualElements.isEmpty()) {
            for (int i = 0; i < this.visualElements.size(); i++) {
                ...
                if (this.visualElements.get(i) == null)
                    continue;

                float childHeight = this.visualElements.get(i).getH();
                this.visualElements.get(i).setY(this.h / 2 - childHeight / 2);

                this.visualElements.get(i).setX(runningCount);
                runningCount += this.visualElements.get(i).getW();
